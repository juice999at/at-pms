...
const Dashboard: React.FC<DashboardProps> = ({ rooms, guests, settings, onBedAction, onQuickBook, onExtendStay }) => {
  ...
  const stats = useMemo(() => {
    const dailyRevenue = rooms.reduce((sum, r) => sum + r.beds.filter(b => b.status === BedStatus.OCCUPIED).reduce((s, b) => s + b.pricePerNight, 0), 0);
    ...
    return { revenue: dailyRevenue, totalRevenue, occupancy, dirtyBeds };
  }, [rooms, guests]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="今日实时营收" value={`¥${stats.revenue}`} color="bg-emerald-500" />
        <StatCard title="累计实收总额" value={`¥${stats.totalRevenue}`} color="bg-indigo-500" />
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center space-x-2 bg-slate-50 p-1 rounded-2xl border border-slate-100">
                ...
                <div className="flex bg-white rounded-xl p-0.5 shadow-sm">
                  {['不限', '男', '女'].map(g => (
                    <button key={g} onClick={() => setFilterGender(g as any)} className={`px-3 py-1 rounded-lg text-xs font-black transition-all ${filterGender === g ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                      {g}
                    </button>
                  ))}
                </div>
                  ...
                  <div className="flex space-x-2">
                    <button 
                      onClick={() => setConfirmState({
                        ...
                        title: '确认退房',
                        message: `是否立即为「${guest.name}」及其同行人员办理退房？此操作将释放所有关联床位。`
                      })}
                    ...
                    <button 
                      onClick={() => setConfirmState({
                        ...
                        targetId: guest.id,
                        title: '续住处理',
                        message: `为「${guest.name}」及其同行人员办理延期。`,
