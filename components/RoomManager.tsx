    ...
    <div className="space-y-4 md:space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white p-4 md:p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-stretch md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as any)} className="flex-1 md:flex-none px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none font-black text-slate-600">
            <option value="ALL">全部房型</option>
            <option value={RoomType.STANDARD}>标准间</option>
            <option value={RoomType.SUPERIOR}>豪华间</option>
          </select>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="flex-1 md:flex-none px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none font-black text-slate-600">
            <option value="ALL">全部状态</option>
            <option value={BedStatus.AVAILABLE}>待预订</option>
            <option value={BedStatus.OCCUPIED}>已入住</option>
          </select>
        </div>
        <button onClick={() => setShowBatchModal(true)} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-sm font-black shadow-lg hover:bg-indigo-700 active:scale-95 transition-all">
          批量添加房间
        </button>
      </div>
        ...
        {filteredRooms.map((room) => (
          <div key={room.id} className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all flex flex-col">
            <div className="bg-slate-50/80 px-8 py-6 border-b border-slate-200 flex justify-between items-center">
              ...
              <button onClick={() => onDeleteRoom(room.id)} className="text-slate-300 hover:text-red-500 p-2.5 hover:bg-red-50 rounded-2xl transition-all">
            ...
            <div className="p-6 space-y-4 flex-1">
              {room.beds.map((bed) => {
                const guest = guests.find(g => g.id === bed.guestId);
                // 根据性别决定颜色
                const genderColorClass = guest?.gender === '男' 
                ...
                return (
                  <div key={bed.id} className={`group p-5 rounded-[2rem] border-2 transition-all ${
                    bed.status === BedStatus.OCCUPIED ? 'border-slate-100 bg-slate-50/20' : 'border-slate-50 bg-slate-50/30 hover:border-slate-100'
                  }`}>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center space-x-5">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-md transition-all group-hover:scale-110 ${genderColorClass}`}>
                          {bed.name.slice(-1)}
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-3">
                        ...
                        <div className="flex items-center space-x-1.5">
                          {bed.status === BedStatus.AVAILABLE ? (
                            <button onClick={() => onOpenBooking(bed.id)} className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 shadow-lg active:scale-90 transition-all">
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
                            </button>
                          ) : (
                            <button onClick={() => onBedAction(bed.id, { status: BedStatus.AVAILABLE })} className="p-2.5 bg-rose-100 text-rose-600 rounded-xl hover:bg-rose-200 active:scale-90 transition-all">
                          ...
                          <button onClick={() => onBedAction(bed.id, { cleaningStatus: bed.cleaningStatus === CleaningStatus.CLEAN ? CleaningStatus.DIRTY : CleaningStatus.CLEAN })} className={`p-2.5 rounded-xl active:scale-90 transition-all border ${
...
const BatchAddModal = ({ onClose, onConfirm }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-md rounded-[3rem] p-10 space-y-8 shadow-2xl">
        ...
        <div className="space-y-5">
          <div className="space-y-1.5">
            ...
            <input type="number" value={data.start} onChange={e => setData({...data, start: +e.target.value})} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold" />
          </div>
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => onConfirm(data.start, data.count, data.beds, data.type)} className="flex-1 py-5 bg-indigo-600 text-white rounded-[2rem] font-black shadow-xl active:scale-95 transition-all">确认生成</button>
        </div>
      </div>
    </div>
  );
